...
from trader.models import TickerInfo, StockMarketData
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            df = yf.download(tickers=tickers, period='2d', interval='1m', group_by='ticker', threads=True)
            entries = self.process_entries(df)
            print(len(entries), 'entries to create.')
            StockMarketData.objects.bulk_create(entries, 900)

            for ticker in tickers:
                query = StockMarketData.objects.all().filter(ticker=ticker).order_by('date_time').only('close')
    ...
    def process_entries(self, df):
        ...
        for ticker, data in df.groupby(level=0, axis=1):
            for date_time, row in data.iterrows():
                entry = StockMarketData(ticker=ticker,
                                        date_time=date_time,
